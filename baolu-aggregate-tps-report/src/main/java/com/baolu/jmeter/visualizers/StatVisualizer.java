...
package com.baolu.jmeter.visualizers;
...
public class StatVisualizer extends AbstractVisualizer implements Clearable, ActionListener {
    ...
    private static final String CALC_TPS = "calcTps"; //$NON-NLS-1$
     ...
     * 聚合报告控制吞吐量统计为TPS
     * add by libaolu
     * calcTps 默认true统计成TPS,false 统计成吞吐量
     */
    private final JCheckBox calcTps = new JCheckBox(
            "Calc Tps",true); //$NON-NLS-1$
    ...
    public String getStaticLabel(){
        return "Baolu Aggregate TPS Report";
    }

    @Override
    public String getLabelResource() {
        return super.getClass().getSimpleName();  //$NON-NLS-1$
    }

    @Override
    public void add(final SampleResult res) {
        /*if (res.isSuccessful()){
            if (!res.getSampleLabel().endsWith("-ok")){
                res.setSampleLabel(res.getSampleLabel()+"-ok");
            }
        }else {
            if (!res.getSampleLabel().endsWith("-ko")){
                res.setSampleLabel(res.getSampleLabel()+"-ko");
            }
        }*/
        SamplingStatCalculator row = tableRows.computeIfAbsent(
                ...
                label -> {
                    /**
                     * 此处屏蔽调用官方默认的 SamplingStatCalculator(String) 构造器
                     * 改成调用新编写的 SamplingStatCalculator(String,boolean) 构造器
                     */
                    /*SamplingStatCalculator newRow = new SamplingStatCalculator(label);*/
                    SamplingStatCalculator newRow = new SamplingStatCalculator(label,calcTps.isSelected());
                    newRows.add(newRow);
                    return newRow;
                });
        synchronized (row) {
    ...
    public void clearData() {
        synchronized (lock) {
            ...
            tableRows.clear();
            newRows.clear();
            tableRows.put(TOTAL_ROW_LABEL, new SamplingStatCalculator(TOTAL_ROW_LABEL,calcTps.isSelected()));
    ...
    public void modifyTestElement(TestElement c) {
        ...
        /*add by libaolu*/
        c.setProperty(CALC_TPS, calcTps.isSelected(), true);
    }
    ...
    public void configure(TestElement el) {
        ...
        /*add by libaolu*/
        calcTps.setSelected(el.getPropertyAsBoolean(CALC_TPS, true));
    }
